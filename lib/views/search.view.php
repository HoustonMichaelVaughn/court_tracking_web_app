...
<form method="get" action="<?= dirname($_SERVER['SCRIPT_NAME']) ?>/defendant/search">
    ...
    <?php $safeQuery = $query['q'] ?? ''; ?>
    <input type="text" name="q" value="<?= htmlspecialchars($safeQuery, ENT_QUOTES, 'UTF-8') ?>" placeholder="Search term..." />
...
<?php elseif (!empty($query['q'] ?? '')): ?>
    <p>No results found for "<strong><?= htmlspecialchars($query['q'] ?? '', ENT_QUOTES, 'UTF-8') ?></strong>"
